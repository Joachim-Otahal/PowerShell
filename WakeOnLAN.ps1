...
function WakeOnLan {
    ...
    for ($j = 0 ; $j -lt $Tries ; $j++) {
        ...
        if (($j+1) -lt $Tries) {Start-Sleep -Milliseconds 200} # has to be 200 due to socket.Close() behaviour.
    }
}

...
Write-Verbose "Export SMB Event as $Env:USERPROFILE\Desktop\SMB-Events $Env:COMPUTERNAME $(Get-Date -Format "yyyy-MM-dd").csv" -Verbose
...
#    Sort-Object IpAddress -Unique | 
    Export-Csv "$Env:USERPROFILE\Desktop\SMB-Events $Env:COMPUTERNAME $(Get-Date -Format "yyyy-MM-dd").csv" -Encoding UTF8 -NoTypeInformation -Delimiter ";"
...
# Evaluate result
Write-Verbose "Export as XML for later reevaluation to $Env:USERPROFILE\Desktop\NTLM-Events $Env:COMPUTERNAME $(Get-Date -Format "yyyy-MM-dd").xml" -Verbose
# Save as "raw data" to reimport and reevaluate for more details later.
$EventsNTLM | Export-Clixml "$Env:USERPROFILE\Desktop\NTLM-Events $Env:COMPUTERNAME $(Get-Date -Format "yyyy-MM-dd").xml"
...
Write-Verbose "Export as CSV to $Env:USERPROFILE\Desktop\NTLM-Events $Env:COMPUTERNAME $(Get-Date -Format "yyyy-MM-dd").csv" -Verbose
